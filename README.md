...
관리자 화면(`/admin.html`)에 로그인 → **① 문제 편집** 에서 수정합니다. 오른쪽
미리보기에 즉시 반영되고, **[저장하기]** 를 누르면 참가자 화면에 바로 적용됩니다.

- **단어/힌트**: 각 줄에서 정답과 힌트(설명)를 입력
- **방향**: `가로/세로` 버튼으로 토글
- **위치 이동**: `◀ ▲ ▼ ▶` 버튼으로 그리드 위에서 칸 단위로 이동
- **+ 단어 추가 / 삭제**: 행 추가·삭제
- **🧩 자동 정렬**: 서로 안 이어지는 단어가 붙지 않도록(빈 칸 확보) 한 번에 정리
- **💡 글자 힌트 토글**: 참가자 화면의 "글자 힌트" 버튼(랜덤 한 글자 공개) ON/OFF
- 서로 이어지지 않는 단어가 맞붙으면 **빨간 칸**으로 경고하고, 교차 글자가
  어긋나도 알려줍니다.

> 번호는 위치(읽기 순서)에 따라 자동으로 매겨집니다.

### 참가자 화면 기능
- **시작하기**: 이름·소속 입력 후 시작 → 퍼즐 공개 + 풀이 시간 타이머
- **입력**: 글자를 채우면 다음 칸으로 자동 이동, 현재 칸·단어가 강조됩니다
- **키보드**: 방향키 이동 · Tab(진행 방향 이동) · Backspace/Delete로 지우기
- **💡 글자 힌트**: 누르면 전체 맵에서 임의의 한 글자를 공개(관리자가 켰을 때)
- **교차 칸**: 두 단어가 겹치는 칸은 정답 글자가 미리 채워져 있습니다
- 모두 맞히면 **컨페티** 축하 효과 🎉

### 관리자 로그 기능
- **정렬**: 제출 순서(선착순) ↔ 풀이 시간(빠른순)
- **🎲 랜덤 뽑기**: 제출자 중 무작위 당첨자 추첨
- **CSV 내려받기 / 순위 초기화**
...
3. 참가자: 링크 접속 → **[시작하기 🚀]** (이때부터 퍼즐 공개 + 풀이 시간 측정) →
   퍼즐 풀기 → **[정답 확인]** → 이름·소속 입력 → **[정답 제출하기]**
   - 보라색 칸은 미리 채워진 **힌트 글자**(교차 지점)입니다.
   - 제출하면 본인 화면에 "○등으로 기록되었습니다! (풀이 시간 ○○:○○)" 가 표시됩니다.
4. 진행자: 관리자 화면 **② 정답 로그** → **[새로고침]** → 1등(🏆)부터 확인
   - **정렬**을 *제출 순서(선착순)* ↔ *풀이 시간(빠른순)* 으로 바꿔볼 수 있습니다.
5. **[CSV 내려받기]** 로 전체 명단을 엑셀로 저장(현재 정렬 기준 그대로)
6. 다음 회차/재시작 시 **[순위 초기화]** 로 로그를 깨끗이 비웁니다(되돌릴 수 없음).

---

## 6-1. 로고 교체 (CJ ENM 공식 로고로)

현재 로고(`assets/logo.svg`)는 이 작업 환경에서 cjenm.com 접속이 차단되어
**임시로 재현한 SVG**입니다. 공식 로고로 바꾸려면:

- 가장 쉬운 방법: 받은 공식 로고 파일을 `assets/logo.svg` 이름으로 **덮어쓰기**
- PNG 로 바꾸려면: 파일을 `assets/logo.png` 로 넣고, `index.html`·`admin.html`
  상단의 `<img class="logo" src="assets/logo.svg" ...>` 에서 `src` 를
  `assets/logo.png` 로 변경
- 색상 톤을 조정하려면 `css/style.css` 상단의 `--brand`, `--brand2` 값을 변경
